...
void playerDeleteFromMapOfGames(PlayerID player_id, Map games){
    if(!player_id || !games){
        return;
    }
...
ChessResult chessRemovePlayer(ChessSystem chess, int player_id){
    ...
    MAP_FOREACH(char*, game_id, player_games) {
        ...
        if (!tournamentIsOver(current_tournament)) {
            ...
            gameSetWinner(current_game_in_player, (gameGetPlayer1ID(current_game_in_player), new_player_id) == 0 ? SECOND_PLAYER : FIRST_PLAYER);
    ...
    MAP_FOREACH(int*, tournament_id, chess->tournaments) {
        ...
        if (!tournamentIsOver(current_tournament)) {
            ...
            if(mapGetSize((tournament_games))>0){
                playerDeleteFromMapOfGames(new_player_id, tournament_games);
                MAP_FOREACH(char*, game_id, tournament_games) {
                    ...
                    gameSetWinner(current_game_in_tournament, (gameGetPlayer1ID(current_game_in_tournament), new_player_id) == 0 ? SECOND_PLAYER : FIRST_PLAYER);
